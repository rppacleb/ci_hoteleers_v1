$(document).ready(function(){
    ...
    $('#side_manage_dropdown').css({'opacity': 0.8, "background-color": "rgb(0, 41, 170)"});
    //end highlight
    ...
    load_record(0,'education','education_container','education_pagination','education_loader');
    load_record(0,'job_type','job_type_container','job_type_pagination','job_type_loader');
    load_record(0,'industry','industry_container','industry_pagination','industry_loader');
    load_record(0,'department','department_container','department_pagination','department_loader');
    load_record(0,'job_level','job_level_container','job_level_pagination','job_level_loader');
    load_record(0,'location','location_container','location_pagination','location_loader');
    load_record(0,'perks_and_benefits','perks_and_benefits_container','perks_and_benefits_pagination','perks_and_benefits_loader');
    load_record(0,'employer_position','employer_position_container','employer_position_pagination','employer_position_loader');
    //load record
                    ...
                    $.each(data.data, function (key, value) {
                        ...
                        html += '<div class="col ">';
                            ...
                            //html += '<div class="alert bg-transparent alert-dismissible fade show shadow" role="alert">';
                              html += '<small><a style="text-decoration:none;" href="#/" data-toggle="modal" data-target="#edit_modal" data-title="'+type+'" data-type="'+type+'" data-id="'+value.id+'" class="'+txt_color+' edit_link" aria-type="'+type+'" aria-id="'+value.id+'" aria-inactive="'+(value.inactive)+'">'+value.name+'</a></small>';                              
                              html += '<button title="Remove" class="text-danger close remove" aria-type="'+type+'" aria-id="'+value.id+'">&times;</button>';
        ...
        html += '<form id="frm_'+param.type+'_edit">';
            ...
            html += '<div class="form-group row">';
                ...
                html += '<div class="col-10">';
                  html += '<input readonly class="form-control form-control-md mb-2" type="text" aria-type="'+param.type+'" name="header[id]"/>';
                html += '</div>';
            html += '</div>';
            ...
            html += '<div class="form-group row">';
                ...
                html += '<div class="col-10">';
                  html += '<input maxlength="200" class="form-control form-control-md mb-2" type="text" name="header[name]"/>';
                html += '</div>';
            html += '</div>';
    ...
    function load_data(param){
        ...
        $.ajax({
            ...
            if(data.data !== null){
                ...
                $('#frm_'+param.type+'_edit input[name=header\\[name\\]]').val(data.data.name);
                data.data.inactive = parseInt(data.data.inactive);
                $('#frm_'+param.type+'_edit input[name=header\\[inactive\\]]').prop('checked',data.data.inactive);
