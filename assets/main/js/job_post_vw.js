      ...
      .done(function (response) {
        if (response.data !== null) {
          ...
          if (param.mode == "edit" || param.mode == "view") {
            ...
            const qualification = $(
              "#frm_data_entry textarea[name=header\\[qualification\\]]"
            ).jqte({
              // b: false,
              br: false,
              center: false,
            ...
            if (param.mode == "view") {
              $(
                "#frm_data_entry select[name=header\\[salary_type\\]]"
              ).addClass("d-none");
            } //end if

            if (param.mode == "view") {
              $("#frm_data_entry input[name=header\\[salary_from\\]]").val(
                toCurrency(data.salary_from)
              );
              $("#frm_data_entry input[name=header\\[salary_to\\]]").val(
                toCurrency(data.salary_to)
              );
              if (data.salary_from <= 0 && data.salary_to <= 0) {
                $("#frm_data_entry input[name=header\\[salary_from\\]]")
                  .parent()
                  .parent()
                  .addClass("d-none");
              } //end if
