...
$(document).ready(function(){
    ...
    if(job_post !== ""){
      ...
      $('input[id=header\\[job_post_text\\]]').val(job_post).prop('disabled',true); 
    }//end if
    ...
    //load record
    $(document.body).on('blur',frm_1+' input[name=header\\[id\\]]',function(e){
        var id          = $(this).val();
        var param       = {};
        param["id"]     = id;
        param["mode"]   = 'view';
    ...
    //load record
    $(document.body).on('blur',frm_2+' input[name=header\\[id\\]]',function(e){
        var id          = $(this).val();
        var param       = {};
        param["id"]     = id;
        param["mode"]   = 'view';
                                ...
                                html += '<div class="card">';
                                    html += '<div class="card-body">';
                                        html += '<div class="row">';
                                            ...
                                            //html += '</div><!--./col-->';
                                            html += '<div class="col">';
                                                html += '<a href="'+applicant_url+value.user_id+'?job_post='+value.job_post_id+'" class="text-link">'+value.name+'</a>';
                                            html += '</div><!--./col-->';
                                            ...
                                            html += '<div class="col-2">';
                                                ...
                                                html += '<a href="'+edit_url+value.id+'?user_id='+value.user_id+'&job_post='+value.job_post_id+''+'" class="mr-1 btn btn-pill-sm btn-pill-sm-outline-light text-muted btn_save"><i class="fa fa-pencil-alt"></i></a>';
                                                html += '<a href="'+view_url+value.id+'?user_id='+value.user_id+'&job_post='+value.job_post_id+''+'" class="btn btn-pill-sm btn-pill-outline-link text-link btn_view">View</a>';
    ...
    $(document.body).on('focus','input[id=header\\[job_post_text\\]]',function(){
        if(!$(this).hasClass('wauto')){
            load_job_title_autocomplete($(this),$('input[name=header\\[job_post\\]]'));
            $(this).addClass('wauto');
        }//End if
    })//focus
    $(document.body).on('blur','input[id=header\\[job_post_text\\]]',function(){
        if($(this).val() == ""){
    ...
    $(document.body).on('focus','input[name=header\\[applicant1_text\\]]',function(){
        //if(!$(this).hasClass('wauto')){

            load_applicant_autocomplete($(this),$('input[name=header\\[applicant1\\]]'));
            $(this).addClass('wauto');
        //}//End if
    })//focus
    $(document.body).on('blur','input[name=header\\[applicant1_text\\]]',function(){
        if($(this).val() == ""){
    ...
    //load applicant autocomplete
    $(document.body).on('focus','input[name=header\\[applicant2_text\\]]',function(){
        //if(!$(this).hasClass('wauto')){

            load_applicant_autocomplete($(this),$('input[name=header\\[applicant2\\]]'));
            $(this).addClass('wauto');
        //}//End if
    })//focus
    $(document.body).on('blur','input[name=header\\[applicant2_text\\]]',function(){
        if($(this).val() == ""){
    ...
    //find 1
     $(document.body).on('click','button.btn_find1',function(){
        ...
        $(frm_1+' input[name=header\\[id\\]]').val(id).trigger('blur');
    ...
    //find 2
     $(document.body).on('click','button.btn_find2',function(){
        ...
        $(frm_2+' input[name=header\\[id\\]]').val(id).trigger('blur');
    ...
    function load_data(frm1,param){
        ...
        $.ajax({
            ...
            beforeSend : function(){
                ...
                btn.html('<div class="spinner-grow" role="status"> <span class="sr-only">Loading...</span> </div>');
            },
            ...
            if(response.data !== null && response.data !== undefined){
                ...
                if(param.mode == 'edit' || param.mode == 'view'){
                    ...
                    //----------------------------------------------------------------------------------------------------
                        ...
                        $(frm1 +' input[name=file\\[old_doc_image\\]]').val(data.doc_image);

                        var full_name = data.first_name + ' ' + data.last_name;
                        $(frm1 +' h6[id=header\\[company_name\\]]').html(full_name);
                        $(frm1 +' p[id=header\\[date_created\\]]').html('<small class="text-muted">Joined '+data.joined_date+'</small>');
                        if(data.doc_image === null || data.doc_image == ""){
                            $(frm1 +' img[id=header\\[company_logo\\]]').attr('src',urlimgdef); 
                        }else{
                            $(frm1 +' img[id=header\\[company_logo\\]]').attr('src',upload_path+data.doc_image);
                        }//end if
                        ...
                        html += '<div class="col mb-3">';
                            ...
                            html += '<p class="text-center"><a href="'+upload_path+data.resume+'" target="_blank">'+data.resume+'</a></p>';
                        html += '</div><!--./col-->';
                        if(data.resume !== ""){
                        ...
                        //primary info
                        $(frm1 +' input[name=header\\[first_name\\]]').val(data.first_name);
                        $(frm1 +' input[name=header\\[middle_name\\]]').val(data.middle_name);
                        ...
                        $(frm1 +' input[name=header\\[contact_number\\]]').val(data.contact_number);
                        $(frm1 +' textarea[name=header\\[highlights\\]]').val(data.highlights);
                        data.internship = parseInt(data.internship);
                        $(frm1 +' input[name=header\\[internship\\]]').prop('checked',data.internship);
                        //end primary info

                        if (!$(frm1 +' input[name=header\\[internship\\]]').prop('checked')) {
                            if (!$(frm1 +' input[name=header\\[internship\\]]').prop('checked') && $('.frm_data_entry2 input[name=header\\[internship\\]]').prop('checked')) {
                        ...
                        if (!$('.frm_data_entry2 input[name=header\\[internship\\]]').prop('checked')) {
                            if ($(frm1 +' input[name=header\\[internship\\]]').prop('checked') && !$('.frm_data_entry2 input[name=header\\[internship\\]]').prop('checked')) {
                                $('.ca-internship2').addClass('d-none');
                                $('.ca-internship').removeClass('d-none');
                            } else {
                        ...
                        //location
                        $(frm1 +' p[id=header\\[location\\]]').html('<small class="text-muted">'+data.locality+'</small>');

                        if(frm1 == '.frm_data_entry2'){
                            
                            $(frm1 +' textarea[name=header\\[location2\\]]').val(data.location);
                            
                        }else{
                            
                            $(frm1 +' textarea[name=header\\[location\\]]').val(data.location);
                        }//end if
                            ...
                            html += '<div class="row mb-2">';
                                ...
                                html += '<div class="col">';   
                                    ...
                                    html += '<div class="form-group row d-none">';
                                        html += '<label class="col-5 col-form-label text-link">Line</label>';
                                        html += '<div class="col">';
                                    ...
                                    html += '<div class="form-group row">';
                                        html += '<label class="col-4 col-form-label text-right align-self-center">Position </label>';
                                    ...
                                    html += '<div class="form-group row">';
                                        html += '<label class="col-4 col-form-label text-right align-self-center">Company Name </label>';
                                        html += '<div class="col align-self-center">';
                                    ...
                                    html += '<div class="form-group row">';
                                        html += '<label class="col-4 col-form-label text-right">Short description</label>';
                                        html += '<div class="col">';
                                            html += '<textarea class="form-control form-control-sm ca-textarea" name="row_experience[short_description][]" maxlength="600">'+data_lines[key]['short_description']+'</textarea>';
                                        html += '</div><!--./col-10-->';
                                    html += '</div><!--./form-group-->';

                                    if(data_lines[key]['if_current'] == 0){
                                        html += '<div class="form-group row">';
                                            html += '<label class="col-4 col-form-label text-right align-self-center">Start Date</label>';
                                            html += '<div class="col align-self-center">';
                                        ...
                                        html += '<div class="form-group row">';
                                            html += '<label class="col-4 col-form-label text-right align-self-center">End Date</label>';
                                            html += '<div class="col align-self-center">';
                                    ...
                                    html += '<div class="form-group row d-none">';
                                        html += '<label class="col-5 col-form-label text-link">If Current</label>';
                                        html += '<div class="col">';
                    ...
                    for(var key in data_lines){
                        html += '<div class="row mb-2">';
                             html += '<div class="col">';
                                ...
                                html += '<div class="form-group row">';
                                    html += '<label class="col-4 col-form-label text-right align-self-centerk">University/School </label>';
                                    html += '<div class="col align-self-center">';
                                ...
                                html += '<div class="form-group row">';
                                    html += '<label class="col-4 col-form-label text-right align-self-center">Degree/Field of Study </label>';
                                    html += '<div class="col align-self-center">';
                                ...
                                html += '<div class="form-group row">';
                                    html += '<label class="col-4 col-form-label text-right align-self-center">Education </label>';
                    ...
                    for(var key in data_lines){
                        if(key == 0){
                            html += '<label class="col-form-label text-link">Industry </label>';
                        }//end if
                        html += '<div class="row mb-2">';
                            html += '<div class="col">';
                                html += '<div class="form-group row d-none">';
                    ...
                    for(var key in data_lines){
                       if(key == 0){
                            html += '<label class="col-form-label text-link">Job Level</label>';
                       }//end if
                       html += '<div class="row mb-2">';
                            html += '<div class="col">';
                    ...
                    for(var key in data_lines){
                        if(key == 0){
                            html += '<label class="col-form-label text-link">Job Type</label>';
                        }//end if
                    ...
                    for(var key in data_lines){
                        if(key == 0){
                            html += '<label class="col-form-label text-link">Department</label>';
                        }//end if
                        html += '<div class="row mb-2">';
                            html += '<div class="col">';
                                html += '<div class="form-group row d-none">';
                ...
                if(param.mode == 'view'){
                    $(frm1 +' input:not([readonly]),'+frm1+' textarea:not([readonly]),'+frm1+' select,button').addClass(param.mode);
                    ...
                    $(frm1 +' button.'+param.mode+','+frm1+' input.'+param.mode+','+frm1+' textarea.'+param.mode+','+frm1+' select.'+param.mode+'').removeClass('form-control');
                    $(frm1 +' button.'+param.mode+','+frm1+' input.'+param.mode+','+frm1+' textarea.'+param.mode+','+frm1+' select.'+param.mode+'').addClass('form-control-plaintext');

                    $(frm1 +' button.'+param.mode+', input.'+param.mode+',textarea.'+param.mode+','+frm1+' select.'+param.mode+'').prop('disabled',true);
                    ...
                    if(frm1 == '.frm_data_entry2'){
                        $(frm1 +' input#search_location2').parent().parent().addClass('d-none');
                        $(frm1 +' textarea[name=header\\[location2\\]]').parent().parent().removeClass('d-none');
                        $(frm1 +' textarea[name=header\\[location2\\]]').addClass('form-control-plaintext');
                        $(frm1 +' textarea[name=header\\[location2\\]]').removeClass('form-control');
                        autosize($('.ca-textarea'));
                    }else{
                        $(frm1 +' input#search_location').parent().parent().addClass('d-none');
                        $(frm1 +' textarea[name=header\\[location\\]]').parent().parent().removeClass('d-none');
                        $(frm1 +' textarea[name=header\\[location\\]]').addClass('form-control-plaintext');
                        $(frm1 +' textarea[name=header\\[location\\]]').removeClass('form-control');
                        autosize($('.ca-textarea'));
                    }//end if
