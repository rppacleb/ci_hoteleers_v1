...
$(document).ready(function(){
    ...
    load_record(0,'employer','main_container','main_pagination','main_loader', function(){
    ...
    function load_record(page,type,container,pagination,loader,callback){
        ...
        var data            = {};
            ...
            if($('select[id=header\\[education\\]]').val() !== null && $('select[id=header\\[education\\]]').val() !== ""){
        ...
        $.ajax({
            ...
            dataType: "JSON",
            beforeSend : function(){
                $('#'+loader+'').html('<div class="col-auto"></div><div class="col text-center"><div class="spinner-grow text-muted"></div><div class="col-2"></div></div>');
            },
        }).done(function (data) {
                ...
                if(data.data !== null){
                    ...
                    $.each(data.data, function (key, value) {
                        ...
                        if ($(window).width() <= 480) {
                            html += '<div class="col-12 mb-4" style="min-width:2.5in; margin-left: auto; margin-right: auto;" >';
                            ...
                            html += '<div class="card-body">';
                              ...
                              if(job_title_placeholder_length > 45){
                                  ...
                                  html += '<a href="'+view_url+value.id+'" style="text-decoration:none;"><h6 class="card-title text-link">'+job_title_placeholder+'</h6></a>';
                              ...
                              if(company_placeholder_length > 45){
                                  ...
                                  html += '<small class="text-muted" style="font-size:9pt !important"><b>'+company_placeholder+'</b></small>';
  
                                  html += '</div>';
                                   html += '<div class="mb-0">';
                                      html += '<small class="text-muted" style="font-size:9pt !important"><b>'+value.location_placeholder+'</b></small>';
                                  html += '</div>';
                                  ...
                                  if (parseInt(value.applied)) {
                                    html += '<div class="row mb-0">';
                                        html += '<div>';
                                            html += '<small style="background-color:#FFD580;padding:4px;border-radius:4px;font-size:8pt !important;">Applied on '+value.date_applied+'</small>';
                                        html += '</div>';
                                    html += '</div>';
                                  }
                                  ...
                                  html += '<div class="row mb-1">';
                                      html += '<div class="col">';
                                          html += '<div>';
                                              html += '<small class="text-muted text-left" style="font-size:8pt !important">Apply Before '+value.job_expiration_date+'</small>';
                                          html += '</div>';
                                      html += '</div>';
                                  html += '</div>';
                                  ...
                                  html += '<div class="row mb-1">';
                                      html += '<div class="col">';
                                          html += '<div>';
                                              html += '<small class="text-muted text-left" style="font-size:8pt !important">'+value.job_type_text+'</small>';
                                          html += '</div>';
                                      html += '</div>';
                                  html += '</div>';
                                  ...
                                  html += '<div class="row mb-2">';
                                      html += '<div class="col align-self-center">';
                                          ...
                                          if(user_id !== 0){
                                              ...
                                              if(profile['user_type'] !== undefined && (profile['user_type'] !== 'employer' && profile['user_type'] !== 'admin')){
                                                html += '<button class="mr-3 btn '+btn_attr+' btn_add_fav" aria-saved="'+parseInt(value.saved)+'" aria-id="'+value.id+'">'+logo+'</button>';
                                              }//end if
                                              
                                          }else{
                                            
                                              html += '<a href="'+not_loggedin+value.id+'" class="mr-3 btn '+btn_attr+'" aria-saved="'+parseInt(value.saved)+'" aria-id="'+value.id+'">'+logo+'</a>';
                          ...
                          $('#'+container+'').append(html);
                            ...
                            html += '<div class="row mb-2" style="min-width:2.5in; margin-left: auto; margin-right: auto;">';
                            html += '<div class="col"style="min-height:2.5in;">';
                                html += '<div class="card card-hover">';
                                    html += '<img onClick="window.location.href=\''+logo_url+value.employer+'\'" src="'+img_src+'" class="card-img-top js-cardimg " style="height:300px; padding: 0.75rem;" class="card-img-top" />';
                                    html += '<div class="card-body ">';
                                        ...
                                        html += '<h6><a class="card-title text-link card-link" href="'+view_url+value.id+'" style="text-decoration:none;">'+value.job_title+'</a></h6>';
                                        

                                        html += '<div class="row" style="font-size:10pt;">';
                                            html += '<div class="col col-md-8 mt-2">';
                                                html += '<a href="'+baseurl+'/company_info_public/view/'+value.employer+'" class="text-link text-decoration-none card-link"><small<b>'+value.company_name+'</b></small></a>';
                                            html += '</div>';
                                            html += '<div class="col">';
                                                html += '<div class="text-right">';
                                        ...
                                        html += '<div class="row mb-3" style="font-size:10pt;">';
                                            html += '<div class="col-9 col-md-6">';
                                                html += '<small class="text-muted"><b>'+value.location_placeholder+'</b></small>';
                                            html += '</div>';
                                            html += '<div class="col">';
                                                html += '<div class="text-right">';
                                                    html += '<small class="text-primary text-right"><b class="text-muted">'+value.job_expiration_date+'</b></small>';
                                                html += '</div>';
                                            html += '</div>';
                                        html += '</div>';
                                        ...
                                        if (parseInt(value.applied)) {
                                            html += '<div class="row">';
                                                html += '<div class="col-auto">';
                                                    html += '<small style="background-color:#FFD580;padding:4px;border-radius:4px;">Applied on '+value.date_applied+'</small>';
                                                html += '</div>';
                                            html += '</div>';
                                        }//end if
    ...
    $(document.body).on('click','li.page-item',function(e){
        ...
        if(page == 'next'){
            ...
            load_record(page,type,type+'_container',type+'_pagination',type+'_loader',function(){
    ...
    $(document.body).on('change','select[name=header\\[sort\\]]',function(e){
        ...
        if($(this).val() == 'best_match'){
            if(profile["industry_count"] <= 0 || profile["job_level_count"] <= 0 || profile["job_type_count"] <= 0 || profile["department_count"] <= 0){
                ...
                $.notify.addStyle('warning', {
                    html: "<div class='row justify-content-center'> \
                                <div class='col my-3'> \
                                    ...
                                    <a class='text-primary' href='"+redirect_url+"'>Click here</a> \
                                </div> \
                            </div>",
                    classes: {
                      base: {
            ...
            $.ajax({
                ...
                beforeSend : function(){
                    ...
                    if (data.success === true){
                        ...
                        load_record(0,'signup','main_container','main_pagination','main_loader',function(){
    ...
    function load_dropdown(url,dropdown,type){
        ...
        $.ajax({
            ...
            var html = '';
            if(type == 'education'){
                html += '<option value="" disabled selected hidden>select education</option>';
            }//end if
            for(var key in response.data){
                //html += '<option value="'+response.data[key].id+'">'+response.data[key].name+'</option>';

                if(type == 'years'){
                    html += '<option value="'+response.data[key].year+'">'+response.data[key].name+'</option>';
                }else if(type == 'location'){
                    html += '<option value="'+response.data[key].name+'">'+response.data[key].name+'</option>';
                }else if(type == 'education'){

                    html += '<option value="'+response.data[key].level+'">'+response.data[key].name+'</option>';
                }else{
                    html += '<option value="'+response.data[key].id+'">'+response.data[key].name+'</option>';    
                }
