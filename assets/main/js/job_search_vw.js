...
$(document).ready(function(){
   ...
   //Initialize date time picker
    ...
    //rich text editor
    var job_description =  $('#frm_data_entry textarea[name=header\\[job_description\\]]').jqte({
        // b: false,
        br: false,
        //center: false,
        color: false,
    ...
    var qualification =  $('#frm_data_entry textarea.texteditor[name=header\\[qualification\\]]').jqte({
        // b: false,
        br: false,
        //center: false,
        color: false,
    ...
    $(document.body).on('click','button.btn_submit',function(e){
        var view_url     = homeurl + '/private/view/';
        var redirect_url = baseurl + '/applicant_info/edit/'+profile_id+'?info=1';
            ...
            if(response.data !== null){
                ...
                if(param.mode == 'edit' || param.mode == 'view'){
                    ...
                    //----------------------------------------------------------------------------------------------------
                        ...
                        $('p[id=header\\[company_name\\]]').html('<a href="'+baseurl+'/company_info_public/view/'+data.id+'" class="text-link text-decoration-none">'+data.company_name+'</a>');
                        $('p[id=header\\[recruiter\\]]').html('<small class="text-muted">Recruiter was active '+data.recruiter+'</small>');
                        
                       
                        $('input[name=placeholder\\[report_modal_job_title\\]]').val(data.job_title);
                        ...
                        //primary info
                        $('#frm_data_entry input[name=header\\[date_created\\]]').val(data.date_created);
                        ...
                        $('#frm_data_entry div[name=placeholder\\[job_description\\]]').html(data.job_description);
                        $('#frm_data_entry div[name=placeholder\\[qualification\\]]').html(data.qualification);
                        ...
                        if(data.salary_from <= 0 && data.salary_to <= 0){
                            ...
                            $('#frm_data_entry input[name=header\\[salary_type\\]]').val(data.salary_type.charAt(0).toUpperCase() + data.salary_type.slice(1));
                            $('#frm_data_entry input[name=header\\[salary\\]]').val(data.salary_currency+' '+data.salary_from+' - '+data.salary_currency+' '+data.salary_to);  
                        }
                        ...
                        for(var key in perks_and_benefits2){
                            ...
                            for(var key2 in perks_and_benefits){
                                ...
                                if(perks_and_benefits2[key] == perks_and_benefits[key2].id){
                                    ...
                                    p_b_html += '<div class="col mb-2" style="font-size: 0.875rem;">';
                        ...
                        $('#frm_data_entry input[name=header\\[website\\]]').addClass('d-none');
                        $('#frm_data_entry a[name=placeholder\\[website\\]]').removeClass('d-none');
                        $('#frm_data_entry a[name=placeholder\\[website\\]]').text(data.website);

                        data.website = data.website.replace(/https:\/\//g,'');
                        data.website = data.website.replace(/http:\/\//g,'');
                        $('a[name=placeholder\\[website\\]]').attr('href','https://'+data.website);
                        //end last
                        ...
                        for(var key in data_lines){
                            ...
                            if ($(window).width() <= 480) {
                                html += '<img src="'+src+'" class="img-fluid rounded" style="width: 310px; height: 240px;">';
                            } else {
                        ...
                        if (window.location.href.indexOf("view") >= 0) {
                            if ($('#image_cont_mobile').html() === "" && $('#image_cont_desktop').html() === "") {
