  ...
  function initMap() {
      ...
      */
      ...
      const options = {
        ...
        //types: ['(cities)'],
        types: ['geocode'],
        componentRestrictions: {country: ["us","ph","id","th"]},
        strictBounds: false,
      };
    ...
    autocomplete.addListener("place_changed", () => {
        ...
        $(frm+' textarea[name=header\\[location\\]]').val(place.formatted_address);
        $(frm+' input[name=header\\[lat\\]]').val(place.geometry['location'].lat());
        $(frm+' input[name=header\\[lng\\]]').val(place.geometry['location'].lng());
        $(frm+' input[name=header\\[locality\\]]').val(locality);
        $(frm+' input[name=header\\[administrative_area_level_1\\]]').val(administrative_area_level_1);
        ...
        $('#'+name+'_cont').find(".edit_link").each(function(){
            var value = $(this).text();
            if(value.toLowerCase() === place.formatted_address.toLowerCase()){
                dup = 1;
                return false;
            }//end if
        });
        ...
        if(dup == 0){
            var html_txt = '';
                html_txt += '<div class="col ">';
                    html_txt += '<button type="button" title="Remove" class="text-danger close remove_append">&times;</button>';
                    html_txt += '<small class="d-none"><a style="text-decoration:none;" class="edit_link_locality">'+locality+'</a></small>';
                    html_txt += '<small class="d-none"><a style="text-decoration:none;" class="edit_link_administrative_area_level_1">'+administrative_area_level_1+'</a></small>';
                    html_txt += '<small class="d-none"><a style="text-decoration:none;" class="edit_link_country">'+country+'</a></small>';

                    html_txt += '<small><a style="text-decoration:none;" class="edit_link">'+place.formatted_address+'</a></small>';                              
                    
                html_txt += '</div>';         
            $('#'+name+'_cont').append(html_txt);
            $('input[id=search_location]').val('');
            $('input[id=search_location]').focus();
...
$(document).ready(function(){
    ...
    //highlight
    if(mod_type == 'side_deactivated' || mod_type == 'side_hired_jobs' || mod_type == 'side_insight'){
        mod_type = 'side_insight';
        ...
        $('#side_active_jobs').css({'opacity': 0.8, "background-color": "rgb(0, 41, 170)"});
    }
    ...
    if ($(window).width() <= 1359 && $(window).width() >= 1200) {
        $('.aja-cbcounter').css({'padding': '1rem 0.5rem'});
    }
    

    var homeurl     = baseurl + '/active_jobs_applicant';
    var homeurl_txt = 'active_jobs_applicant';
    ...
    function load_record(page,type,container,pagination,loader){
        var edit_url = baseurl + '/applicant_info/view/applied/';
        ...
        var data                    = {};
            ...
            data.job_title_label    = job_title;
        ...
        var view_url = baseurl + '/applicant_search/view/applied/';
            if(data.view_type == 'leads'){
                view_url = baseurl + '/talent_database/view/';
            }//end if
            ...
            //alert(JSON.stringify(data.years));
                ...
                if(data.data !== null){
                    ...
                    $.each(data.data, function (key, value) {
                        ...
                        html += '<div class="row mb-2">';
                            html += '<div class="col">';
                                var border = 'badge badge-pill';

                                if(value.interview_status !== undefined){
                                    if(value.interview_status == 'pending'){
                                        border = 'badge-warning';
                                    }else if(value.interview_status == 'cancelled'){
                                        border = 'badge-danger';
                                    }else{
                                    ...
                                    html += '<div class="card-body ">';
                                        html += '<div class="row">';
                                            ...
                                            html += '<div class="col-1" style="min-width:1in;">';
                                                //html += '<img data-toggle="tooltip" data-placement="left" title="ID : '+value.id+'" class="rounded-circle" style="height:0.7in;width:0.7in;" src="'+img_src+'" alt="Card image cap">';
                                                html += '<img class="rounded-circle" style="height:0.7in;width:0.7in;" src="'+img_src+'" alt="Card image cap">';
                                            html += '</div><!--./col-->';
                                            html += '<div class="col align-self-center" style="min-width:1.7in;">';
                                                html += '<a href="'+view_url+value.id+'?job_post='+job_post_id+'&mod_type='+mod_type+'" style="color: #3f5d97;text-decoration: none;"><b>'+value.name+'</b></a>';
                                            html += '</div><!--./col-->';
                                            html += '<div class="col align-self-center">';
                                                html += value.location_placeholder;
                                            html += '</div><!--./col-->';
                                            if ($(window).width() <= 1600 && $(window).width() >= 1380) {
                                                var job_title_placeholder           = value.current_job.substring(0,13);
                                                ...
                                                if(job_title_placeholder_length > 13){
                                                    ...
                                                    html += '<div class="col align-self-center"><h5 style="font-size: 15px; font-weight: 400; font-family: Be Vietnam Pro, sans-serif; margin-top: auto; margin-bottom: auto;" data-ajacurrjob="'+value.current_job+'">';
                                                ...
                                                if(company_placeholder_length > 15){
                                                    ...
                                                    html += '<div class="col align-self-center"><h5 style="font-size: 15px; font-weight: 400; font-family: Be Vietnam Pro, sans-serif; margin-top: auto; margin-bottom: auto;" data-ajacompany="'+value.company_name+'">';
                                                ...
                                                var job_title_placeholder           = value.current_job.substring(0,12);
                                                var job_title_placeholder_length    = value.current_job.length;
                        
                                                if(job_title_placeholder_length > 12){
                                                    job_title_placeholder = job_title_placeholder+'...';
                                                    html += '<div class="col align-self-center"><h5 style="font-size: 15px; font-weight: 400; font-family: Be Vietnam Pro, sans-serif; margin-top: auto; margin-bottom: auto;" data-ajacurrjob="'+value.current_job+'">';
                                                        html += job_title_placeholder;
                                                    html += '</h5></div><!--./col-->';
                                                ...
                                                var company_placeholder           = value.company_name.substring(0,13);
                                                var company_placeholder_length    = value.company_name.length;
                        
                                                if(company_placeholder_length > 13){
                                                    company_placeholder = company_placeholder+'...';
                                                    html += '<div class="col align-self-center"><h5 style="font-size: 15px; font-weight: 400; font-family: Be Vietnam Pro, sans-serif; margin-top: auto; margin-bottom: auto;" data-ajacompany="'+value.company_name+'">';
                                                        html += company_placeholder;
                                                    html += '</h5></div><!--./col-->';
                                                ...
                                                var job_title_placeholder           = value.current_job.substring(0,11);
                                                var job_title_placeholder_length    = value.current_job.length;
                        
                                                if(job_title_placeholder_length > 11){
                                                    job_title_placeholder = job_title_placeholder+'...';
                                                    html += '<div class="col align-self-center"><h5 style="font-size: 15px; font-weight: 400; font-family: Be Vietnam Pro, sans-serif; margin-top: auto; margin-bottom: auto;" data-ajacurrjob="'+value.current_job+'">';
                                                        html += job_title_placeholder;
                                                    html += '</h5></div><!--./col-->';
                                                ...
                                                var company_placeholder           = value.company_name.substring(0,11);
                                                var company_placeholder_length    = value.company_name.length;
                        
                                                if(company_placeholder_length > 11){
                                                    company_placeholder = company_placeholder+'...';
                                                    html += '<div class="col align-self-center"><h5 style="font-size: 15px; font-weight: 400; font-family: Be Vietnam Pro, sans-serif; margin-top: auto; margin-bottom: auto;" data-ajacompany="'+value.company_name+'">';
                                                        html += company_placeholder;
                                                    html += '</h5></div><!--./col-->';
                                            ...
                                            html += '<div class="text-center col-xl-2 col-lg-2 align-self-center" style="min-width:1.6in;">';
                                                //html += '<div class="btn-group btn-group-toggle">';
                                                    ...
                                                    //html += '<button class="mr-3 btn btn-pill-sm '+btn_attr+' btn_add_fav" aria-saved="'+parseInt(value.saved)+'" aria-id="'+value.id+'"><i class="fa fa-heart"></i></button>';

                                                    html += '<button aria-saved="'+(value.profile_saved)+'" aria-id="'+value.id+'" class="mr-2 btn '+btn_class+' btn_save_profile">'+logo+'</button>';
                                                    html += '<a href="'+view_url+value.id+'?job_post='+job_post_id+'&mod_type='+mod_type+'" class="btn btn-pill-sm btn-pill-outline-link text-link btn_view">View</a>';
                                        ...
                                        if(value.interview_status !== undefined){
                                            html += '<div class="row justify-content-end align-items-center">';
                                                html += '<div class="col-auto">';
                                                    html += '<label class="col-form-label">Interview Status : </label>';
                                                html += '</div><!--./col-->';
                                                html += '<div class="col-auto">';
                                                    html += '<span class="badge '+border+'">' + toPascalCase(value.interview_status) + '</span>';
                                                html += '</div><!--./col-->';
                                            html += '</div><!--./row-->';
                                        }//end if
                                        ...
                                        if(data["data_notification"] !== undefined){
                                            ...
                                            if(data["data_notification"][value.id] !== undefined){
                                                ...
                                                html += '<div class="row justify-content-start mt-3">';
                                                    html += '<div id="'+value.id+'" class="col-auto">';
                                                    ...
                                                    if(data["data_notification"] !== undefined){
                                                        if(data["data_notification"][value.id] !== undefined){
                                                            ...
                                                            for(const k in notification_record){
                                                                ...
                                                                html += "<span class='"+class_status+" text-break'>Invited to "+m_data["job_title"].toLowerCase()+" | "+toPascalCase(m_data["status"])+"</span><br/>";
                                                            }//end for
                                                        }//end if
                                                    }//end if
                                                    //end notification
                ...
                if(checked){
                    ...
                    $('option:selected', $('#header\\['+type+'\\]')).each(function(element2) {
                      ...
                      html += '<div class="col d-none">';
                            html += '<small class="d-none"><a style="text-decoration:none;" class="edit_link">'+$(this).val()+'</a></small>'; 
                            html += '<small><a style="text-decoration:none;">'+$(this).val()+'</a></small>';                              
                            html += '<button type="button" title="Remove" class="text-danger close remove_append" aria-id="'+$(this).val()+'">&times;</button>';
                        html += '</div>';
                    });
                    ...
                    var html = '';
                    //alert(element.val());
                    $('#'+type+'_cont').find(".edit_link").each(function(){
        ...
        $.ajax({
            ...
            for(var key in response.data){
                if(response.data[key].job_title !== undefined){
                    ...
                    if(type == 'years'){
                        html += '<option value="'+response.data[key].from_range+'-'+response.data[key].to_range+'">'+response.data[key].name+'</option>';
