   ...
   //Initialize date time picker
    ...
    $(document.body).on('click','li.page-item',function(e){
        ...
        if(page == 'next'){
            ...
            load_active_job_posts(page,'company_info_public',type+'_container',type+'_pagination',type+'_loader'); 
        }
    ...
    function load_data(param){
        ...
        $.ajax({
            ...
            if(response.data !== null){
                ...
                if(param.mode == 'edit' || param.mode == 'view'){
                    ...
                    //----------------------------------------------------------------------------------------------------
                        ...
                        var website = $('#frm_data_entry input[name=header\\[website\\]]').val();
                        $('.cipw-website').html('<a href='+website+' target="_blank" class="cipw-websiteval text-link">'+website+'</a>');
            ...
            load_active_job_posts(0,'company_info_public','main_container','main_pagination','main_loader');
    ...
    function load_active_job_posts(page,type,container,pagination,loader){
        ...
        var data            = {};
            data.page       = page;
            data.sort       = "";
            data.keyword    = $('#frm_data_entry input[name=header\\[company_name\\]]').val();
                    ...
                    $.each(data.data, function (key, value) {
                        ...
                        if ($(window).width() <= 480) {
                              ...
                              if(company_placeholder_length > 45){
                                  ...
                                  if (parseInt(value.applied)) {
                                    html += '<div class="row mb-0">';
                                        html += '<div>';
                                            html += '<small style="background-color:#FFD580;padding:4px;border-radius:4px;font-size:8pt !important;">Applied</small>';
                                        html += '</div>';
                                    html += '</div>';
                                  }
                          ...
                          $('#'+container+'').append(html);
                            ...
                            html += '<div class="col-6"style="min-width:2.5in; max-width:5.5in;">';
                                html += '<div class="card flex-row mb-2"style="min-height:2.5in;">';
                                        ...
                                        if (parseInt(value.applied)) {
                                            html += '<div class="row">';
                                                html += '<div class="col-9">';
                                                    html += '<h6><a class="card-title text-link text-decoration-none card-link" style="font-weight:bold" href="'+view_url+value.id+'">'+value.job_title+'</a></h6>';
                                                html += '</div>';
                                                html += '<div class="col-3">';
                                                    html += '<div class="text-right">';
                                                        html += '<small style="background-color:#FFD580;padding:4px;border-radius:4px;">Applied</small>';
                                                    html += '</div>';
                                                html += '</div>';
                                            html += '</div>';
                                        }else{
                                            html += '<h6><a class="card-title text-link card-link" href="'+view_url+value.id+'" style="text-decoration:none;font-weight:bold">'+value.job_title+'</a></h6>';
                                        }
