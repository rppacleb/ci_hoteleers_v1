$(document).ready(function () {
  ...
  var homeurl = baseurl + "/saved_jobs";
  var homeurl_txt = "saved_jobs";
    ...
    $.ajax({
      ...
      .done(function (data) {
        if (data.data !== null) {
          ...
          current_item_count = data.total_result;
            ...
            if ($(window).width() <= 480) {
              ...
              if ($(window).width() <= 480 && $(window).width() >= 376) {
                html +=
                  '<img src="' +
                  img_src +
                  '" class="card-img-top" style="height:330px; padding: 0.75rem;" />';
              } else if ($(window).width() <= 375) {
                html +=
                  '<img src="' +
                  img_src +
                  '" class="card-img-top" style="height:290px; padding: 0.75rem;" />';
              }
              ...
              //html += '<h6 class="card-title text-link">'+value.job_title+'</h6>';
              ...
              html +=
                ...
                value.employer +
                '" class="text-link text-decoration-none"><small<b>' +
                value.company_name +
                "</b></small></a>";
              html += "</div>";
              html += '<div class="col">';
              ...
              html += '<div class="text-right">';
              html +=
                '<small class="text-muted text-right"><b>' +
                value.job_expiration_date +
                "</b></small>";
              html += "</div>";
              html += "</div>";
  ...
  $(document.body).on("click", "li.page-item", function (e) {
    ...
    if (page == "next") {
      ...
      current_active_page = parseInt($(".page-item.active").attr("aria-page"));
      load_record(
        page,
        type,
        type + "_container",
  ...
  $(document.body).on("click", "button.btn_add_fav", function (e) {
    ...
    var basic_btn = "btn-pill-outline-link text-link";
    var saved_btn = "btn-primary";
  ...
  function reload_displayed_saved_jobs() {
    var active_page = parseInt($(".page-item.active").attr("aria-page"));
    isNaN(active_page)
      ? (active_page = current_active_page)
      : (current_active_page = active_page);

    current_item_count--;
    var calculated_page_count = parseInt(current_item_count / items_per_page);
    if (current_item_count % items_per_page > 0) {
      calculated_page_count++;
    }

    // Rerender the pagination buttons.
    var param = {};
    param.total_page = calculated_page_count;
    param.page = active_page - 1;
    param.pagination = "main_pagination";
    load_pagination(param);

    if (active_page > calculated_page_count || active_page === 1) {
      active_page--;
      current_active_page = active_page;
    }
    if ($(".page-item.active").length === 0) {
      $('li[aria-page="' + active_page + '"]').addClass("active");
    }
    load_record(
      active_page,
      "employer",
