$(document).ready(function () {
        ...
        if (data.data !== null) {
          ...
          $.each(data.data, function (key, value) {
            ...
            if (type !== "user_archived") {
              html +=
                '<a style="text-decoration:none;" class="text-link" href="' +
                user_view_url +
                value.id +
                "?employer=" +
                employer +
            ...
            html += "</div><!--./col-->";
            html +=
              '<div class="text-right col-xl-2 col-lg-2 col-md-12 col-sm-12 col-xs-12 align-self-center" style="min-width:2in;">';
            ...
            if (type !== "user_archived") {
              ...
              html +=
                ...
                side_menu +
                add_url +
                '" class="mr-2 btn btn-pill-sm btn-pill-outline-link text-link btn_view">Edit</a>';
              html +=
                '<button aria-id="' +
                value.id +
                '" class="btn btn-pill-sm btn-pill-outline-primary text-primary btn_delete">Delete</button>';
            } //end if
  ...
  $(document.body).on("click", ".btn_deactivate, .btn_pause", function (e) {
    ...
    var data =
      $("#frm_data_entry").serialize() + "&header[process_type]=" + type;
    ...
    if (type == "renew") {
      ...
      window.location.replace(
        edit_url + id + "?side_menu=" + side_menu + "&action=renew"
      );
    } else {
  ...
  $(document.body).on("click", ".btn_archive", function (e) {
    ...
    if (
      confirm(
        "Archiving this employer will remove all the user linked to it. Are you sure you want to continue?"
      )
    ) {
      $.ajax({
        url: url,
      ...
      .done(function (response) {
        if (response.data !== null) {
          ...
          if (param.mode == "edit" || param.mode == "view") {
            ...
            $("#frm_data_entry select[name=signup\\[honorifics\\]]").val(
              data.honorifics
            );
            $("#frm_data_entry input[name=signup\\[first_name\\]]").val(
              data.first_name
            );
            $("#frm_data_entry input[name=signup\\[last_name\\]]").val(
              data.last_name
            );
            $("#frm_data_entry input[name=signup\\[designation\\]]").val(
              data.designation
            );
            $("#frm_data_entry input[name=signup\\[work_email\\]]").val(
              data.work_email
            );
            $("#frm_data_entry select[name=signup\\[dial_code\\]]").val(
              data.dial_code
            );
            $("#frm_data_entry input[name=signup\\[contact_number\\]]").val(
              data.contact_number
            );
            //end contact person
          ...
          if (param.mode == "view") {
            $(
              "#frm_data_entry > .edit_view input:not([readonly]),#frm_data_entry > .edit_view textarea:not([readonly]),#frm_data_entry > .edit_view select,button"
            ).addClass(param.mode);
            ...
            $("#frm_data_entry > .edit_view .input-group-append").addClass(
              "d-none"
            );
            $(
              "#frm_data_entry > .edit_view select." + param.mode + ""
            ).removeClass("custom-select");
            $(
              "#frm_data_entry > .edit_view input." +
                param.mode +
                ",#frm_data_entry > .edit_view textarea." +
                param.mode +
                ",#frm_data_entry > .edit_view select." +
                param.mode +
                ""
            ...
            $(
              "#frm_data_entry > .edit_view input[name^=placeholder]"
            ).removeClass("d-none");
            //$('#frm_data_entry > .edit_view button').addClass('d-none');

            $(
              "#frm_data_entry > .disable_view input:not([readonly]),#frm_data_entry > .disable_view textarea:not([readonly]),#frm_data_entry > .disable_view select,button"
            ).addClass(param.mode);
            $("#frm_data_entry > .disable_view .input-group-append").addClass(
              "d-none"
            );
            $(
              "#frm_data_entry > .disable_view select." + param.mode + ""
            ).removeClass("custom-select");
            $(
              "#frm_data_entry > .disable_view button." +
                param.mode +
                ",#frm_data_entry > .disable_view input." +
                param.mode +
                ",#frm_data_entry > .disable_view textarea." +
                param.mode +
                ",#frm_data_entry > .disable_view select." +
                param.mode +
                ""
            ).removeClass("form-control");
            $(
            ...
            $(
              "#frm_data_entry > .disable_view input[name^=placeholder]"
            ).removeClass("d-none");
