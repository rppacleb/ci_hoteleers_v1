...
$(document).ready(function(){
   ...
   //Initialize date time picker
            ...
            $.ajax({
                ...
                beforeSend : function(){
                    ...
                    if (data.success === true){
                        ...
                        view_url += data.data.id+'?user_id='+user_id+'&job_post='+job_post_id;
    ...
    $(document.body).on('change','select[name=header\\[interview_type\\]]',function(){
    ...
    function load_data(param){
        ...
        var url         = homeurl + '/load_data/'+param.id+'?user_id='+param.user_id+'&job_post='+param.job_post_id;
        ...
        $.ajax({
            ...
            if(response.data !== null){
                    ...
                    //----------------------------------------------------------------------------------------------------
                        var status_html = '';
                            ...
                            if(data.status == 'completed'){
                                status_html = '<span class="badge rounded-pill bg-success">'+data.status.toUpperCase()+'</span>';
                            }else if(data.status == 'cancelled'){
                                status_html = '<span class="badge rounded-pill bg-danger">'+data.status.toUpperCase()+'</span>';
                            }else{
                                status_html = '<span class="badge rounded-pill bg-warning ">'+data.status.toUpperCase()+'</span>';
                            }//end if
                        ...
                        if(data.notif_status !== null){
                            $('h4[name=placeholder\\[notif_status\\]]').html(toPascalCase(data.notif_status));
                            $('h4[name=placeholder\\[notif_status\\]]').addClass(resp_class);
                        }//end if
                        

                        $('#frm_data_entry input[name=header\\[interview_date\\]]').val(data.interview_date);

                        $('#frm_data_entry input[name=placeholder\\[interview_date\\]]').val(data.interview_date_placeholder);

                        $('#frm_data_entry input[name=header\\[interview_start_time\\]]').val(data.interview_start_time);
                        $('#frm_data_entry input[name=header\\[interview_end_time\\]]').val(data.interview_end_time);

                        if(response.placeholder !== null){
                            var placeholder     = response.placeholder[0];
                            $('#frm_data_entry input[name=placeholder\\[applicant_name\\]]').val(placeholder.applicant_name);
                            $('#frm_data_entry input[name=placeholder\\[applicant_email\\]]').val(placeholder.applicant_email);
                            $('#frm_data_entry input[name=placeholder\\[company_name\\]]').val(placeholder.company_name);
                            $('#frm_data_entry input[name=placeholder\\[job_title\\]]').val(placeholder.job_title);
                        }//end if

                        $('#frm_data_entry input[name=header\\[interview_end_time\\]]').val(data.interview_end_time);
                        $('#frm_data_entry input[name=header\\[interviewer_name_position\\]]').val(data.interviewer_name_position);
                        $('#frm_data_entry select[name=header\\[interview_type\\]]').val(data.interview_type).trigger('change');

                        $('#frm_data_entry input[name=header\\[virtual_interview_link\\]]').val(data.virtual_interview_link);
                        $('#frm_data_entry textarea[name=header\\[notes_to_interviewee\\]]').val(data.notes_to_interviewee);
                ...
                if(param.mode == 'view'){
                    ...
                    $('#frm_data_entry button.'+param.mode+':not(".show_on_view"), input.'+param.mode+',textarea.'+param.mode+',select.'+param.mode+'').prop('disabled',true);
                    
                    //placehoder
                    $('#frm_data_entry input[name^=placeholder]').removeClass('d-none');
                    $('#frm_data_entry input[name=header\\[interview_date\\]]').addClass('d-none');
                    //end placehoder
                    ...
                    if($('#frm_data_entry select[name=header\\[interview_type\\]]').val() == 'face_to_face'){
