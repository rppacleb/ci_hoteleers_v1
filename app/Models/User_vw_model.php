...
class User_vw_model extends Model {
    ...
    function load_data($id,$request){
            ...
            if($request['is_archived'] == 'T'){
                $qrystr     = "SELECT t0.*,t1.*
                            FROM ousr_archive t0
    ...
    function get_employer($request){
            ...
            if(isset($request['employer'])){
                $qrystr .= " AND t0.id = '".$request['employer']."'";
            }
            $qrystr .= " ORDER BY t0.id ASC";
        ...
        */
        ...
        if(isset($data['record_profile'])){
            if(count($data['record_profile']) > 0){
                ...
                $builder->where('id', $data['record_profile']['id']);
                $builder->update();
                $profile_id = $data['record_profile']['id'];
