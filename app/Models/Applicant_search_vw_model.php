...
class Applicant_search_vw_model extends Model {
    ...
    function load_data($id,$request){
            ...
            $qrystr     = "SELECT t2.name AS 'full_name',
                                    ...
                                    t3.job_title,
                                    IFNULL((SELECT a.status 
                                         FROM job_post_move_to a 
                                         WHERE a.id = t0.id 
                                         AND a.user_id = t0.user_id 
                                         AND a.if_current = true 
                                         LIMIT 1),'') AS 'status',
                                     
                                     IFNULL((SELECT a.id 
                                         FROM job_post_for_interview a
                                         WHERE a.id = t0.id
                                         AND a.user_id = t0.user_id 
                                         AND STR_TO_DATE(CONCAT(a.interview_date, ' ',a.interview_end_time), '%m/%d/%Y %h:%i %p') > NOW()
                                         LIMIT 1),0) AS 'is_interviewed',
            ...
            if(isset($request['job_post']) && $request['job_post'] !== ""){
                $qrystr .= " AND t0.id = ".$request['job_post']."";
            }//end if

            if(isset($request['employer_id']) && $request['employer_id'] !== ""){
                $qrystr .= " AND t3.employer = ".$request['employer_id']."";
            }//end if
            ...
            $qrystr     = "SELECT   t0.*,
                                    DATE_FORMAT(STR_TO_DATE(t0.start_date,'%m/%d/%Y'), '%m %Y') AS 'start_date',
                                    DATE_FORMAT(STR_TO_DATE(t0.end_date,'%m/%d/%Y'), '%m %Y') AS 'end_date',
                                    DATE_FORMAT(STR_TO_DATE(t0.start_date,'%m/%d/%Y'), '%b %Y') AS 'start_date_placeholder',
                                    DATE_FORMAT(STR_TO_DATE(t0.end_date,'%m/%d/%Y'), '%b %Y') AS 'end_date_placeholder'
            ...
            $qrystr     = "SELECT   t0.*,
                                    DATE_FORMAT(STR_TO_DATE(t0.issued_date,'%m/%d/%Y'), '%b %d, %Y') AS 'issued_date_placeholder',
                                    DATE_FORMAT(STR_TO_DATE(t0.expiration_date,'%m/%d/%Y'), '%b %d, %Y') AS 'expiration_date_placeholder'
            ...
            $qrystr     = "SELECT * FROM 
                            ...
                            SELECT t0.status,
                                    1,
                                    DATE_FORMAT(STR_TO_DATE(t0.interview_date,'%m/%d/%Y'),'%b %d, %Y') AS 'application_date'
                            FROM job_post_for_interview t0
                            WHERE t0.job_post_id = '{$request['job_post']}' 
                            AND t0.user_id = '$id'
                            AND t0.created_by = '{$request['user_id']}'
                            AND (t0.status = 'pending' OR t0.status = 'completed' OR t0.status = 'cancelled')
                            AND STR_TO_DATE(t0.interview_date,'%m/%d/%Y') <= STR_TO_DATE(DATE_FORMAT(NOW(),'%m/%d/%Y'),'%m/%d/%Y')
                            
                            ) x
                            ORDER BY application_date DESC";
            $query      = $this->db->query($qrystr);
            if($query->getNumRows() > 0){
                $response['line_move_to'] = $query->getResultArray();
            }//end if
            //end department
    ...
    function check_interview($param){
        $response = [];
        $response['success'] = false;

        $qrystr = "SELECT t0.* 
            FROM job_post_for_interview t0 
            WHERE t0.job_post_id = '".$param["job_post_id"]."' 
            AND t0.user_id = '".$param["user_id"]."' 
            AND t0.status = 'pending' 
            AND STR_TO_DATE(CONCAT(t0.interview_date, ' ',t0.interview_end_time), '%m/%d/%Y %h:%i %p') > NOW() 
            LIMIT 1";
        $query = $this->db->query($qrystr);
        $response['num_rows'] = $query->getNumRows();
        if($query->getNumRows() > 0){
