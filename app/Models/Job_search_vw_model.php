...
class Job_search_vw_model extends Model {
    ...
    function load_data($id){
            ...
            $qrystr     = "SELECT t1.doc_image,
                                  t1.company_name,
                                    ...
                                    DATE_FORMAT(STR_TO_DATE(t8.last_logged_in,'%m/%d/%Y'), '%b %d, %Y') AS 'recruiter',
    ...
    public function add_record($data){
        ...
        if(isset($data["record_notification"])){
            $builder = $this->db->table('onotification');
            $builder->set($data['record_notification']);
            $builder->where('id', $data['record_notification']['id']);
            $builder->update();
        }//end if
