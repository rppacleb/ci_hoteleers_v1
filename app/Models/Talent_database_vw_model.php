...
class Talent_database_vw_model extends Model {
    ...
    function get_job_post($type,$request,$perPage,$offset){
            $response   = [];
            $qrystr     = "";
            ...
            $qrystr     = "SELECT t1.doc_image,
                                    ...
                                    (SELECT COUNT(DISTINCT(user_id)) FROM job_post_applicant WHERE id = t0.id AND user_id = '".$request['applicant']."') AS 'applicant_count',
                                    (SELECT COUNT(DISTINCT(id)) FROM usr_job_invite WHERE job_post = t0.id AND id = '".$request['applicant']."') AS 'invite_count'
                                FROM ojob_post t0
    ...
    //get email data
    function get_email_data($param){
            $response   = [];
            $qrystr     = "";

            $qrystr     = "SELECT t0.*,t1.company_name
                                FROM ".$param["dup_table"]." t0
                                ".$param["dup_filter"]."";
            $query                      = $this->db->query($qrystr);
            $response['num_rows']       = $query->getNumRows();
