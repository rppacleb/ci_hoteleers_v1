...
use App\Models\Account_management_model;
use App\Models\Audit_trail_model;
use App\Libraries\Pcl_lib;
class Account_management extends BaseController{
	protected $model;
	protected $session;
	...
	public function __construct(){
		...
		$this->model 			= new Account_management_model();
    ...
    public function index(){
		...
		if(!$this->session->has('username')){
			...
			return view('Account_management/index',$param);
  		}//end if
	...
	public function get_notification(){
		...
		$param 			= [];
		//pagination
		$res 			= $this->model->get_notification();
		$res2 			= $this->model->get_notification_counter();
		if($res['num_rows'] > 0){
	    	$param["data"] 			= $res['data'];
	    	if($res2['num_rows'] > 0){
	    		$param["num_rows"] 		= $res2["data"][0]['counter'];
	    	}else{
	    		$param["num_rows"] 		= 0;
	    	}//end if
	...
	//purge data
	public function auto_purge_data(){
		$request = $this->request->getVar();
		$response = array(
			"success" 	=> false,
			"message" => ""
		...
		try{
			//purge record
			$res 		= $this->model->auto_purge_data($request);		
			if(!$res['success']){
