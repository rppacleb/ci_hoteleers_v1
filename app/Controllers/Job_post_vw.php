...
class Job_post_vw extends BaseController{
    ...
    public function index($type,$id){
		...
		if(!$this->session->has('username')){
			...
			return view('Job_post_vw/index',$response);
  		}//end if
        
    }//end function

	public function get_perks_and_benefits(){
		//get perks and benefits
		$param = array();
		$param['table_name'] 		= 'operks_and_benefits';
		$param['filter_field'] 		= 'inactive';
    ...
    public function get_record($table_name){
    	...
    	$res 			= $this->model->get_record($param,$request);
    	if($res['success']){
    ...
    public function get_record2($table_name){
    	$param 			= array();
    	$param['table_name'] 	= $table_name;
    	$response 		= null;

    	$res 			= $this->model->get_record2($param);
    	if($res['success']){
    		$response['data'] = $res['data'];
    	}//end if
    	//$response['data'] = $this->model->get_record($param);
