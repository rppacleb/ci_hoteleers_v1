...
use App\Models\Active_jobs_model;
use App\Models\Audit_trail_model;
use App\Libraries\Pcl_lib;
class Active_jobs extends BaseController{
	protected $model;
	protected $session;
	...
	public function __construct(){
		...
		$this->model 			= new Active_jobs_model();
    ...
    public function index(){
			...
			return view('Active_jobs/index',$param);
    	...
    	$param 				= $this->request->getVar();
			try{
				...
				/*$data['for_update'][] 		= "UPDATE ".$data['table_name']." 
												...
												WHERE id = '".$data['record_header']['id']."'"; //Enable after testing to 1 day*/

			    //remove this after qa												
				$data['for_update'][] 		= "UPDATE ".$data['table_name']." 
												SET remove_on = DATE_FORMAT((DATE_ADD(NOW(), INTERVAL 30 DAY)),'%m/%d/%Y %h:%i %p')
