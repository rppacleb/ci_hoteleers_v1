...
use App\Models\Active_jobs_applicant_model;
use App\Models\Audit_trail_model;
use App\Libraries\Pcl_lib;
class Active_jobs_applicant extends BaseController{
	protected $model;
	protected $session;
	...
	public function __construct(){
		...
		$this->model 			= new Active_jobs_applicant_model();
    ...
    public function index(){
			...
			//check access
  			...
  			if(isset($request['job_post'])){
  				...
  				$res 	= $this->model->get_master_data2($data);
	  			if($res['num_rows'] > 0){
  			...
  			//get counter
  			if(isset($request['job_post'])){
  				$request['job_title_label'] = $param['employer_data'][0]['job_title'];
  				$request['filter_data'] 	= $param['employer_data'][0];
  				$res 	= $this->model->get_counter($request);
  				
	  			if($res['num_rows'] > 0){
	  				$param['counter_data'] = $res['data'];
	  				$param['counter_data_qrystr'] = $res['qrystr'];
	  			}//end if
  			}//end if
			...
			return view('Active_jobs_applicant/index',$param);
    ...
    public function get_record($type){
		...
		if($res['num_rows'] > 0){
	    	...
	    	$param["data"] 			= $res['data'];
			if(isset($res['data_notification'])){
				$param["data_notification"] = $res['data_notification'];
			}//end if
			
	    	$param["qrystr"] 		= $res['qrystr'];
