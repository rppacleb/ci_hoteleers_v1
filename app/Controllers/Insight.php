...
class Insight extends BaseController{
	...
	public function __construct(){
		...
		$this->model 			= new Insight_model();
		$this->audit_trail 		= new Audit_trail_model();
		$this->record_type 		= 'active_jobs';
    ...
    public function index(){
			...
			$res 	= $this->model->get_counter($request);
			if($res['num_rows'] > 0){
				$param['counter_data_closed'] = $res['data'];
			}//end if

			//get counter
  			$request['type'] 		= 'deactivated';
    		$request['employer_id'] = $this->session->get('employer');
    		$request['user_id'] 	= $this->session->get('userid');
			$res 	= $this->model->get_counter($request);
			if($res['num_rows'] > 0){
				$param['counter_data_deactivated'] = $res['data'];
			}//end if
    		...
    		$request['employer_id'] = $this->session->get('employer');
    		$request['user_id'] 	= $this->session->get('userid');
			$res 	= $this->model->get_hiring_speed($request);
			if($res['num_rows'] > 0){
				$param['counter_hiring_speed'] = $res['data'];
			}//end if
			...
			$request['type'] 		= 'active';
    		$request['employer_id'] = $this->session->get('employer');
			$res 	= $this->model->get_origin_counter($request);
			if($res['num_rows'] > 0){
				$param['counter_origin_active'] = $res['data'];
			}
			...
			$res 	= $this->model->get_origin_counter($request);
			if($res['num_rows'] > 0){
				$param['counter_origin_hired'] = $res['data'];
			}
    	
  			
			return view('Insight/index',$param);
	...
	public function get_pie_chart_data(){
		...
		$param 			= [];
		//pagination
		$res 			= $this->model->get_pie_chart_data($request);
		if($res['num_rows'] > 0){
	    	$param["data"] 			= $res['data'];
	    	$param["num_rows"] 		= $res['num_rows'];
		}else{
    ...
    public function remove_job_post(){
    	$validator 			= [];
    	$data 				= array();
    	$param 				= $this->request->getVar();
			try{
