...
use App\Models\Compare_applicant_model;
use App\Models\Audit_trail_model;
use App\Libraries\Pcl_lib;
class Compare_applicant extends BaseController{
	protected $model;
	protected $session;
	...
	public function __construct(){
		...
		$this->model 			= new Compare_applicant_model();
    ...
    public function index(){
			...
			return view('Compare_applicant/index',$param);
	...
	public function get_job_post_list(){
		$request 				= $this->request->getVar();
    	$response 		= null;

    	$res 			= $this->model->get_job_post_list($request);
    	if($res['success']){
    ...
    public function get_applicant_list(){
    	...
    	$res 			= $this->model->get_applicant_list($request);
    	if($res['success']){
    		$response['data'] = $res['data'];
    	}//end if
    	//$response['data'] = $this->model->get_record($param);
			...
			try{
				...
				$data['for_update'][] 		= "UPDATE job_post_move_to 
												...
												AND (SELECT COUNT(id) 
													...
													WHERE status = 'offered' OR status = 'hired') <= 0";
				//end update query
