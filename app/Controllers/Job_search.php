...
use App\Models\Job_search_model;
use App\Models\Audit_trail_model;
use App\Libraries\Pcl_lib;
class Job_search extends BaseController{
	protected $model;
	protected $session;
	...
	public function __construct(){
		...
		$this->model 			= new Job_search_model();
    ...
    public function index($type){
  			...
  			$param['type'] 				= $type;
			$req_param 				= array();
			$req_param['user_id'] 	= $this->session->get('userid');
			$param['profile'] 	    = $this->model->get_profile($req_param);
    	
  			
			return view('Job_search/index',$param);
    ...
    public function add_fav(){
    	...
    	$param 				= $this->request->getVar();
			try{
				...
				$data["dup_filter"] = "t0.id = '".$data['record_header']['id']."' AND t0.job_post";
